...
public class ProtocolConnectionHandler : IProtocolConnectionHandler
{
    ...
    public async Task HandleConnection(ProtocolConnectionContext connectionContext, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            do
            {
                ...
                if (length > 0)
                {
                    _logger.LogDebug("{ClientProtocol}: received {Length} bytes", connectionContext.Protocol, length);

                    int startIndex = GetStartIndex(buffer, length, connectionContext.Protocol);

                    await _handler.HandleMessage(connectionContext, connectionContext.NetworkStream,
        ...
        {
            _logger.LogCritical(exception, "{ClientProtocol}", connectionContext.Protocol);
        }
        finally
        {
            _logger.LogDebug("{ClientProtocol}: disconnected {EndPoint}", connectionContext.Protocol,
                connectionContext.NetworkStream.RemoteEndPoint);
